                ...
                button {
                    ...
                    div { class: "flex items-center gap-3 flex-1",
                        if let Some(pack) = current_soundpack() {
                            ...
                            div { class: "text-base-content/50 text-sm", "Select a mouse soundpack..." }
                        }
                    }
                    ChevronDown {
                        class: format!(
                                            ...
                                            move |_| {
                                                ...
                                                if let Some(pack_item) = soundpacks().iter().find(|p| p.id == pack_id) {
                                                    ...
                                                    spawn(async move {
                                                        ...
                                                        let result = crate::libs::audio::load_mouse_soundpack(&audio_ctx_async, &pack_id_async);

                                                        match result {
                                                            Ok(_) => {
                                                                println!("✅ Mouse soundpack changed to: {} (background loading)", pack_name);
                                                            }
                                                            Err(e) => {
                                                                error_async.set(format!("Failed to load mouse soundpack: {}", e));
                                                                println!("❌ Failed to load mouse soundpack {}: {}", pack_id_async, e);
                                                            }
                                                        }
