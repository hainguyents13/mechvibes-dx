...
use crate::libs::input_device_manager::{ InputDeviceManager, InputDevice };
...
pub fn DeviceSelector(props: DeviceSelectorProps) -> Element {
    ...
    let input_devices = use_signal(|| Vec::<InputDevice>::new());
            ...
            spawn(async move {
                ...
                match device_type {
                    ...
                    DeviceType::Keyboard | DeviceType::Mouse => {
                        match InputDeviceManager::enumerate_devices() {
                            Ok(device_list) => {
                                let filtered_devices: Vec<InputDevice> = device_list
                                    .into_iter()
                                    .filter(|d| {
                                        match device_type {
                                            DeviceType::Keyboard =>
                                                d.device_type.contains("Keyboard"),
                                            DeviceType::Mouse => d.device_type.contains("Mouse"),
                                            _ => false,
                                        }
                                    })
                                    .collect();
                                input_devices.set(filtered_devices);
                            }
                            Err(e) => {
                    ...
                    match props.device_type {
                        ...
                        DeviceType::Keyboard | DeviceType::Mouse => rsx! {
                            if input_devices().is_empty() && !is_loading() {
                                li { 
                                    div { class: "p-4 text-center text-base-content/50 text-sm",
                                        format!("No {} devices found", 
                                            if props.device_type == DeviceType::Keyboard { "keyboard" } else { "mouse" }
                                        )
                                    }
                                }
                            } else {
                                ...
                                for device in input_devices().iter() {
                                    li {
                                        ...
                                        a {
                                            class: format!(
                                                "flex items-center gap-3 {}",
                                                if current_selection().1.contains(&device.instance_id) { "active" } else { "" }
                                            ),
                                            onclick: {
                                                let device_id = device.instance_id.clone();
                                                move |_| {
                                                    handle_device_action.call(device_id.clone());
                                                }
                                            },
                                            ...
                                            if current_selection().1.contains(&device.instance_id) {
                                                Check { class: "w-4 h-4 text-success" }
                                            }
                                        }
                                    }
