...
use lucide_dioxus::{Check, ChevronDown, Keyboard, Music, Search};
use std::sync::Arc;
...
fn KeyboardSelector() -> Element {
    ...
    // Get current keyboard soundpack
    ...
    let current_soundpack = use_memo(move || {
        soundpacks().into_iter().find(|pack| pack.id == current())
    });
                ...
                button {
                    ...
                    div { class: "flex items-center gap-3 flex-1",
                        if let Some(pack) = current_soundpack() {
                            ...
                            div { class: "text-base-content/50 text-sm", "Select a keyboard soundpack..." }
                        }
                    }
                    ChevronDown {
                        class: format!(
                                        ...
                                        onclick: {
                                            ...
                                            move |_| {
                                                ...
                                                if let Some(pack_item) = soundpacks().iter().find(|p| p.id == pack_id) {
                                                    println!("📦 Found keyboard soundpack in cache: {}", pack_item.name);
                                                    let pack_id_clone = pack_id.clone();
                                                    
                                                    // Update keyboard soundpack config
                                                    ...
                                                    spawn(async move {
                                                        ...
                                                        let result = crate::libs::audio::load_keyboard_soundpack(&audio_ctx_async, &pack_id_async);
                                                        
                                                        match result {
                                                            Ok(_) => {
                                                                println!("✅ Keyboard soundpack changed to: {} (background loading)", pack_name);
                                                            }
                                                            Err(e) => {
                                                                error_async.set(format!("Failed to load keyboard soundpack: {}", e));
                                                                println!("❌ Failed to load keyboard soundpack {}: {}", pack_id_async, e);
                                                            }
                                                        }
                                                        is_loading_async.set(false);
                                                    });
                                                } else {
                                                    println!("❌ Keyboard soundpack {} not found in cache", pack_id);
                                                }
                                            }
                                        },
