...
use lucide_dioxus::{Check, ChevronDown, CircleCheck, Keyboard, Music, Search};
use std::sync::Arc;
                          ...
                          move |_| {
                              ...
                              if let Some(pack_item) = soundpacks().iter().find(|p| p.id == pack_id) {
                                  ...
                                  spawn(async move {
                                      ...
                                      let result = crate::libs::audio::load_keyboard_soundpack(
                                          &audio_ctx_async,
                                          &pack_id_async,
                                      );
                                      match result {
                                          Ok(_) => {
                                              println!(
                                                  "✅ Keyboard soundpack changed to: {} (background loading)",
                                                  pack_name,
                                              );
                                          }
                                          Err(e) => {
                                              error_async
                                                  .set(format!("Failed to load keyboard soundpack: {}", e));
                                              println!(
                                                  "❌ Failed to load keyboard soundpack {}: {}",
                                                  pack_id_async,
                                                  e,
                                              );
                                          }
