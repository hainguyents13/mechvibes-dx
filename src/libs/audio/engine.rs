...
impl EngineState {
    ...
    /// Falls back to the system default device, used when the active device
    /// disappears (e.g. USB headset unplugged) rather than a user choice.
    /// Not wired to a caller yet - that needs a cpal stream-error callback
    /// to detect device removal, which is a separate piece of work from the
    /// user-initiated SwitchDevice path this phase implements.
    #[allow(dead_code)]
