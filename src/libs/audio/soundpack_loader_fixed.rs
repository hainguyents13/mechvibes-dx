...
pub fn load_soundpack_optimized(
    context: &AudioContext,
    soundpack_id: &str,
) -> Result<(), String> {
        ...
        if let Some(cached_data) = cache.load_audio_cache(soundpack_id) {
            ...
            match CompressedAudioData::decompress(&cached_data) {
                Ok(audio_data) => {
                    ...
                    println!("✅ Loaded {} from cache ({} samples)", 
...
fn load_and_cache_soundpack_optimized(
    ...
    let soundpack_item = soundpack_cache.get_soundpack_by_id(soundpack_id)
    ...
    let compressed_data = CompressedAudioData::new(
        ...
        soundpack_item.soundpack.version.clone().unwrap_or_else(|| "1.0".to_string()),
    );
    ...
    match compressed_data.compress() {
        Ok(serialized) => {
            ...
            let metadata = create_soundpack_metadata(&soundpack_item.relative_path, &soundpack_item.soundpack)?;
            cache.add_soundpack(metadata);
            cache.save();
            
            let stats = compressed_data.get_stats();
            println!("💾 Cached {} - Original: {}, Compressed: {}, Savings: {}", 
                soundpack_id, 
...
fn update_context_optimized(
    ...
    if let Ok(mut cached) = context.cached_samples.lock() {
        *cached = Some((audio_data.samples, 2, 44100)); // stereo, 44100 Hz
    ...
    if let Ok(mut key_map) = context.key_map.lock() {
        ...
        for (key, mappings) in audio_data.key_mappings {
            let converted_mappings: Vec<[f32; 2]> = mappings.into_iter()
...
fn create_key_mappings(
    ...
    for (key, sound_def) in &soundpack.def {
        ...
        let converted_mappings: Vec<(f64, f64)> = sound_def.iter()
...
pub fn cleanup_cache(keep_recent: usize) -> Result<String, String> {
    ...
    Ok(format!("🧹 Cache cleanup completed, kept {} most recent files", keep_recent))
}
